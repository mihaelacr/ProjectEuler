...
def getNeighbour(i, j, d):
  if i >= size or j >= size:
    raise Exception("Invalid grid indices")
  if d == 0:
    if i < size - 1:
      return (i + 1, j)
    else:
      raise Exception("Reached border, cannot advance to the left!")
  elif d == 1:
    if j < size - 1:
      return (i, j + 1)
    else:
      raise Exception("Reached border, cannot advance to the right!")
  elif d == 2:
